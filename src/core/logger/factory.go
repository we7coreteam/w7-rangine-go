...
type Factory struct {
	driverResolverMap map[string]func(config config.Config) (driver.Driver, error)
...
func NewLoggerFactory() *Factory {
	factory := &Factory{
		...
		driverResolverMap: make(map[string]func(config config.Config) (driver.Driver, error)),
	}
...
func (factory *Factory) MakeDriver(config config.Config) (driver.Driver, error) {
...
func (factory *Factory) RegisterDriver(driver string, resolver func(config config.Config) (driver.Driver, error)) {
...
func (factory *Factory) Register(conf map[string]config.Config) {
	for channelName, channel := range conf {
		func(channelName string, driver config.Config) {

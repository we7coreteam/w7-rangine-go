...
func (self MakeProjectCommand) Configure(cmd *cobra.Command) {
	...
	cmd.Flags().String("name", "", "Project name, such as github.com/we7coreteam/w7-rangine-go/v3-skeleton or w7-rangine-go-skeleton")
...
func (self MakeProjectCommand) Handle(cmd *cobra.Command, args []string) {
	...
	response, _ := http.Get("https://codeload.github.com/we7coreteam/w7-rangine-go/v3-skeleton/zip/refs/heads/main")
	defer response.Body.Close()
	data, _ := io.ReadAll(response.Body)
	if len(data) == 0 {
		cmd.PrintErrln("Error: Failed to download zip file, Please download from https://codeload.github.com/we7coreteam/w7-rangine-go/v3-skeleton/zip/refs/heads/main")
		cmd.Usage()
		return
	}
...
func (self MakeProjectCommand) replaceFileContent(file string, content []byte, name string) string {
	...
	if file == "app/home/provider.go" || file == "go.mod" || file == "main.go" {
		return strings.ReplaceAll(string(content), "github.com/we7coreteam/w7-rangine-go/v3-skeleton", name)
	}
